...
class TestMoveToDevicePassDevice(TestCase):
    ...
    def test_move_device_to(self, device):
        accelerator = torch.device(device)
        device_type = accelerator.type

        class M(torch.nn.Module):
            def forward(self, x):
                x = torch.ops.aten.to.device(x, device=accelerator, dtype=torch.float32)
                return x + x
...
def forward(self, arg0_1):
    ...
    def test_move_to_device_pass(self, device):
        ...
        ep = move_to_device_pass(ep, location=accelerator)
        gm = ep.module()
        test_inputs = (torch.rand(1, 10, 4).to(accelerator),)
        outputs = gm(*test_inputs)
        self.assertEqual(outputs.device, accelerator)
    ...
    def test_move_device_example_inputs(self, device):
        ...
        accelerator = torch.device(device)
        moved_ep = move_to_device_pass(ep, location=accelerator)

        # Verify example_inputs moved to the accelerator
        self.assertEqual(moved_ep.example_inputs[0][0].device, accelerator)
        self.assertEqual(moved_ep.example_inputs[0][1].device, accelerator)
        self.assertEqual(moved_ep.example_inputs[1]["z"].device, accelerator)
